...
# Create a new tunnel
$output = & "$devTunnelPath" create 2>&1
Write-Output $output
    ...
    if ($tunnel_Id) {
        ...
        if ($exit_status -ne 0) {
            ...
            if ($port_number) {
                ...
                $output = & "$devTunnelPath" token --scope connect --json 2>&1
                if ($exit_status -ne 0) {
                    Write-Output "Access token creation failed: $output"
                } else {
                    Write-Output "Access token creation succeeded"
                    $jsonObject = $output | ConvertFrom-Json
                    $token = $jsonObject.token
                    [System.Environment]::SetEnvironmentVariable("TunnelInfo", "https://$tunnel_id.devtunnels.ms:$port_number;$token", "Machine")
                }
            } else {
